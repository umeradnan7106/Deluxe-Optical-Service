  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <section className="bg-[#1B2B5E] py-12 md:py-20 relative overflow-hidden">
        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: "radial-gradient(ellipse 80% 60% at 60% 50%, #C9A84C, transparent)" }} />
        <div className="max-w-[1500px] mx-auto px-4 sm:px-6 text-center relative">
          <p className="text-[#C9A84C] text-[11px] font-semibold uppercase tracking-[.14em] mb-3">Lens Education</p>
          <h1 className="font-playfair text-3xl md:text-5xl lg:text-6xl text-white font-bold mb-4">Lens Guide</h1>
          ...
          {LENS_TYPES.map(({ id, name, color, badge, description, benefits, cta }, idx) => (
            ...
            >
              ...
              <div className={idx % 2 === 1 ? "lg:order-2" : ""}>
                ...
                <h2 className="font-playfair text-3xl text-[#1B2B5E] font-bold mb-4">{name}</h2>
                <p className="text-[#64748b] leading-relaxed mb-6 text-sm">{description}</p>
              ...
              <div className={`rounded-xl p-6 md:p-12 flex items-center justify-center ${idx % 2 === 1 ? "lg:order-1" : ""}`} style={{ backgroundColor: `${color}15`, border: `1px solid ${color}30` }}>
                <div className="text-center">
                  ...
                  <p className="font-playfair text-2xl font-bold text-[#1B2B5E]">{name}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <section className="bg-[#1B2B5E] py-10 md:py-16 relative overflow-hidden">
        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: "radial-gradient(ellipse 60% 60% at 50% 50%, #C9A84C, transparent)" }} />
        <div className="max-w-[1500px] mx-auto px-4 sm:px-6 text-center relative">
          <h2 className="font-playfair text-2xl md:text-4xl text-white font-bold mb-3">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            ...
            <Link href="/contact">
              <Button variant="white" size="lg">Ask an Optician</Button>
            </Link>
          </div>
        </div>
      </section>
