  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-['Cormorant_Garamond'] text-3xl text-white font-semibold">Dashboard</h1>
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-[#1a1a1a] rounded p-5">
            <p className="text-gray-400 text-xs mb-4 uppercase tracking-wide">Orders & Revenue — Last 7 Days</p>
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={stats.orders_7d}>
                ...
                <Tooltip contentStyle={{ background: "#0a0a0a", border: "1px solid #2a2a2a", borderRadius: 4 }} labelStyle={{ color: "#fff" }} />
          ...
          <div className="bg-[#1a1a1a] rounded p-5">
            <p className="text-gray-400 text-xs mb-4 uppercase tracking-wide">Orders by Status</p>
            {pieData.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
            ...
            ) : <p className="text-gray-600 text-sm text-center pt-12">No orders yet</p>}
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-[#1a1a1a] rounded p-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-gray-400 text-xs uppercase tracking-wide">Recent Orders</p>
            <Link href="/admin/orders" className="text-[#E8670A] text-xs hover:underline">View all</Link>
          </div>
          <table className="w-full text-xs">
            <thead><tr className="text-gray-600 text-left border-b border-[#2a2a2a]">
              <th className="pb-2">Order</th><th className="pb-2">Customer</th><th className="pb-2">Total</th><th className="pb-2">Status</th>
            </tr></thead>
            <tbody>
              {recentOrders.slice(0, 6).map((o) => (
                <tr key={o.id} className="border-b border-[#2a2a2a]/50 hover:bg-[#2a2a2a]/20">
                  <td className="py-2 pr-3">
                    <Link href={`/admin/orders/${o.id}`} className="text-[#E8670A] font-mono hover:underline">{o.order_number}</Link>
                  </td>
                  <td className="py-2 pr-3 text-gray-300">{o.customer_name}</td>
                  <td className="py-2 pr-3 text-white">{formatPrice(o.total_amount)}</td>
                  <td className="py-2"><Badge variant={STATUS_BADGE[o.status] ?? "gray"}>{o.status}</Badge></td>
                </tr>
              ))}
              {recentOrders.length === 0 && <tr><td colSpan={4} className="py-4 text-gray-600 text-center">No orders yet</td></tr>}
            </tbody>
          </table>
        </div>
        ...
        <div className="bg-[#1a1a1a] rounded p-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-gray-400 text-xs uppercase tracking-wide">Pending Reviews</p>
            <Link href="/admin/reviews" className="text-[#E8670A] text-xs hover:underline">View all</Link>
          </div>
          <div className="space-y-2">
            {pendingReviews.slice(0, 5).map((r) => (
              <div key={r.id} className="border-b border-[#2a2a2a]/50 pb-2">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-white text-xs font-medium">{r.customer_name}</p>
                    <p className="text-gray-500 text-xs">{r.product_name}</p>
                  </div>
                  <span className="text-[#E8670A] text-xs">{"★".repeat(r.rating)}</span>
                </div>
                <p className="text-gray-400 text-xs mt-0.5 truncate">{r.body}</p>
              </div>
            ))}
            {pendingReviews.length === 0 && <p className="text-gray-600 text-sm text-center py-4">No pending reviews</p>}
          </div>
        </div>

        {/* Low Stock */}
        <div className="bg-[#1a1a1a] rounded p-5 md:col-span-2">
          <div className="flex items-center justify-between mb-3">
            <p className="text-gray-400 text-xs uppercase tracking-wide">Low Stock Alerts</p>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {lowStock.slice(0, 8).map((item) => (
                <tr key={item.variant_id} className="border-b border-[#2a2a2a]/50">
                  <td className="py-2 pr-3 text-white">{item.product_name}</td>
                  <td className="py-2 pr-3 text-gray-300">{item.color_name}</td>
                  <td className="py-2 pr-3 text-gray-500 font-mono">{item.sku_variant ?? "—"}</td>
                  <td className="py-2 pr-3">
                    <span className={item.stock === 0 ? "text-red-400 font-bold" : "text-orange-400"}>{item.stock}</span>
                  </td>
                  <td className="py-2 text-gray-600">{item.low_stock_threshold}</td>
                </tr>
              ))}
              {lowStock.length === 0 && <tr><td colSpan={5} className="py-4 text-gray-600 text-center">All products well stocked</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
