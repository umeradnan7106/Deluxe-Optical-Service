...
export default function AdminReviewsPage() {
  ...
  return (
    <div>
      <h1 className="font-['Cormorant_Garamond'] text-3xl text-white font-semibold mb-6">Reviews</h1>

      <div className="flex gap-2 mb-6">
        {TABS.map(({ key, label }) => (
          <button key={key} onClick={() => setTab(key)}
            className={`px-4 py-2 rounded text-sm transition-colors ${tab === key ? "bg-[#E8670A] text-white" : "bg-[#1a1a1a] text-gray-400 hover:text-white"}`}>
            {label}
          </button>
        ))}
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? Array.from({ length: 5 }, (_, i) => (
              ...
              <tr key={r.id} className="border-b border-[#2a2a2a] hover:bg-[#2a2a2a]/30 transition-colors">
                <td className="px-4 py-3">
                  <p className="text-white">{r.customer_name}</p>
                  <p className="text-gray-500 text-xs">{r.customer_email}</p>
                </td>
                <td className="px-4 py-3 text-gray-300 text-sm">{r.product_name}</td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-1 flex-wrap">
                    ...
                    {r.is_approved && (
                      <button onClick={() => toggleFeature(r.id)} className={`text-xs px-2 py-1 ${r.is_featured ? "text-[#E8670A]" : "text-gray-500 hover:text-white"}`}>
      ...
      {selectedReview && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1a1a1a] rounded w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-4">
              <h3 className="text-white font-semibold text-lg">{selectedReview.title}</h3>
            ...
            <div className="space-y-2 text-sm mb-4">
              ...
              <p className="text-gray-400">By <span className="text-white">{selectedReview.customer_name}</span> · {selectedReview.customer_email}</p>
              <p className="text-gray-400">Product: <span className="text-[#E8670A]">{selectedReview.product_name}</span></p>
              <p className="text-gray-400">Date: {formatDate(selectedReview.created_at)}</p>
              {selectedReview.order_id && <Badge variant="green">Verified Purchase</Badge>}
            </div>
            <p className="text-gray-300 text-sm leading-relaxed mb-4">{selectedReview.body}</p>
            {selectedReview.images?.length > 0 && (
              <div className="flex gap-2 mb-4">
                {selectedReview.images.map((url, i) => (
                  <a key={i} href={url} target="_blank" rel="noopener noreferrer">
                    <Image src={url} alt="" width={80} height={80} className="rounded object-cover border border-[#2a2a2a]" />
                  </a>
                ))}
              </div>
            )}
