...
function TbBtn({ active, onClick, children }: { active?: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      ...
      className={`min-w-[36px] min-h-[36px] px-2 py-1 text-xs rounded transition-colors ${active ? "bg-[#C9A84C] text-white" : "text-gray-500 hover:text-gray-900 hover:bg-gray-100"}`}
    >
      {children}
    </button>
  );
...
export default function ProductForm({ productId }: ProductFormProps) {
  ...
  const [productType, setProductType] = useState<"eyeglasses" | "contact-lenses" | "accessory">("eyeglasses");
  ...
  useEffect(() => {
    ...
    adminApi.products.detail(productId).then(({ data }) => {
      ...
      setForm({
        ...
        slug: p.slug,
        focus_keywords: (p as AdminProductData & { focus_keywords?: string }).focus_keywords ?? "",
      ...
      setVariants(p.variants.map((v, i) => {
        ...
        return {
          ...
          size_label: sz.label,
          lens_width: sz.lw || (p.lens_width_mm ? String(p.lens_width_mm) : ""),
          bridge: sz.br || (p.bridge_mm ? String(p.bridge_mm) : ""),
          temple: sz.tm || (p.temple_mm ? String(p.temple_mm) : ""),
  ...
  async function handleSave() {
    ...
    try {
      ...
      for (const row of variants) {
        const { size_label: sl, lens_width: lw, bridge: br, temple: tm } = row;
        const encodedLabel = lw || br || tm ? `${sl}|${lw}|${br}|${tm}` : (sl || null);
        const vPayload = {
          color_name: row.color_name,
          color_hex: row.color_hex || null,
          size_label: encodedLabel,
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 items-start">
      ...
      <div className="flex-1 min-w-0 w-full">
        ...
        {/* Product Type Tabs */}
        <div className="flex gap-1 bg-[#F8FAFC] border-[1.5px] border-[#E2E8F0] rounded-xl p-1 mb-5 overflow-x-auto scrollbar-none">
          {([
            { key: "eyeglasses" as const, label: "👓 Eyeglasses Frame" },
            { key: "contact-lenses" as const, label: "👁 Contact Lenses" },
            { key: "accessory" as const, label: "🎒 Accessory" },
          ] as { key: typeof productType; label: string }[]).map(({ key, label }) => (
            <button key={key} type="button" onClick={() => { setProductType(key); setField("category", key === "eyeglasses" ? "eyeglasses" : key === "contact-lenses" ? "contact-lenses" : "accessories"); }}
              className={`shrink-0 flex-1 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${productType === key ? "bg-white text-[#1B2B5E] shadow-sm font-semibold" : "text-[#64748b] hover:text-[#1B2B5E]"}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="lg:hidden sticky top-[52px] z-10 bg-white border-b border-gray-200 px-4 py-2 flex items-center justify-between -mx-0 mb-4 shadow-sm">
          ...
          <button type="button" onClick={handleSave} disabled={saving}
            className="shrink-0 bg-[#C9A84C] text-white text-xs font-medium px-3 py-1.5 rounded min-h-[36px] disabled:opacity-60">
            {saving ? "Saving…" : "Save"}
          </button>
        </div>
        ...
        <Section title="Product Images">
          ...
          <div
            className="border-2 border-dashed border-gray-300 rounded p-4 md:p-6 text-center cursor-pointer hover:border-[#C9A84C] transition-colors mb-4"
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 md:gap-3">
                  {imgs.map((img, idx) => (
                    <div key={img._key} className="relative group aspect-square rounded overflow-hidden bg-gray-50 border border-gray-200">
                      ...
                      {idx === 0 && (
                        <span className="absolute bottom-0 left-0 right-0 bg-[#C9A84C] text-white text-[10px] text-center py-0.5">Main</span>
                      )}
        ...
        <Section title="Variants (Colors & Sizes)">
          ...
          <div className="md:hidden space-y-3 mb-3">
              ...
              <div key={row._key} className="bg-white border border-gray-200 rounded-lg p-3">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <input value={row.color_name}
                      ...
                      className="text-sm font-medium text-gray-900 border-b border-transparent focus:border-[#C9A84C] outline-none bg-transparent min-w-0 flex-1"
                  ...
                  <select value={row.size_label}
                    ...
                    className="ml-2 text-xs border border-gray-200 rounded px-1 py-1 text-gray-700 outline-none focus:border-[#C9A84C] min-h-[36px] shrink-0">
                ...
                <div className="flex items-center gap-1 mb-2">
                  ...
                  <input value={row.lens_width}
                    ...
                    className="w-10 border border-gray-200 rounded px-1 py-1 text-xs text-gray-900 text-center outline-none focus:border-[#C9A84C]" placeholder="52" />
                  <span className="text-gray-400 text-xs">□</span>
                  <input value={row.bridge}
                    onChange={(e) => updateVariant(row._key, "bridge", e.target.value)}
                    className="w-10 border border-gray-200 rounded px-1 py-1 text-xs text-gray-900 text-center outline-none focus:border-[#C9A84C]" placeholder="18" />
                  <span className="text-gray-400 text-xs">-</span>
                  <input value={row.temple}
                    onChange={(e) => updateVariant(row._key, "temple", e.target.value)}
                    className="w-12 border border-gray-200 rounded px-1 py-1 text-xs text-gray-900 text-center outline-none focus:border-[#C9A84C]" placeholder="140" />
                </div>
                <input value={row.sku_variant}
                  onChange={(e) => updateVariant(row._key, "sku_variant", e.target.value.toUpperCase())}
                  className="w-full border border-gray-200 rounded px-2 py-1 text-xs text-gray-500 outline-none focus:border-[#C9A84C] mb-2"
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <div>
                    ...
                    <input type="number" value={row.price}
                      ...
                      className="w-full border border-gray-200 rounded px-2 py-1.5 text-sm text-gray-900 outline-none focus:border-[#C9A84C]"
                      placeholder="—" min="0" />
                  </div>
                  <div>
            ...
            <button type="button" onClick={addVariant}
              className="w-full flex items-center justify-center gap-1 text-[#C9A84C] text-sm border border-dashed border-[#C9A84C] rounded-lg py-2.5 hover:bg-orange-50 transition-colors min-h-[44px]">
          ...
          <div className="hidden md:block overflow-x-auto">
            ...
            <button type="button" onClick={addVariant}
              className="mt-3 flex items-center gap-1 text-[#C9A84C] text-xs hover:text-orange-300 transition-colors">
              <PlusIcon className="w-4 h-4" />
              Add Color / Size
            </button>
          </div>
                ...
                <div key={group}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {opts.map((opt) => {
                      ...
                      return (
                        <label key={opt.id}
                          className={`flex items-center gap-2 px-3 py-2 rounded border cursor-pointer text-sm transition-colors ${checked ? "border-[#C9A84C] bg-[#C9A84C]/10 text-gray-900" : "border-gray-200 text-gray-600 hover:border-gray-400"}`}>
                          <input type="checkbox" checked={checked} onChange={() => toggleLensOption(opt.id)} className="accent-[#C9A84C]" />
                          {opt.name}
        ...
        {/* 8. Lens Recommendation Blog Cards */}
        <Section title="Lens Recommendation Blog Links">
          <p className="text-gray-400 text-xs mb-3">Link educational blog posts to show alongside lens options (helps customers choose the right lenses).</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              { icon: "🔵", title: "Blue Cut Lens Guide", desc: "How blue light blocking lenses work", badge: "Prescription Products" },
              { icon: "📖", title: "How to Read Your Prescription", desc: "SPH, CYL, Axis explained simply", badge: "Lens Guide" },
            ].map((card) => (
              <div key={card.title} className="border border-[#E2E8F0] rounded-lg p-3 flex items-start gap-3 hover:border-[#C9A84C] transition-colors cursor-pointer">
                <span className="text-xl shrink-0">{card.icon}</span>
                <div className="min-w-0">
                  <p className="text-sm font-medium text-[#1B2B5E] leading-tight">{card.title}</p>
                  <p className="text-xs text-gray-400 mt-0.5 truncate">{card.desc}</p>
                  <span className="inline-block mt-1 text-[10px] font-semibold bg-[#EEF1FA] text-[#1B2B5E] px-2 py-0.5 rounded">{card.badge}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="w-full lg:w-64 lg:shrink-0 space-y-4">
        <div className="bg-white border border-gray-200 shadow-sm rounded p-4">
          ...
          <label className="flex items-center gap-2 mt-3 cursor-pointer">
            <input type="checkbox" checked={form.is_featured} onChange={(e) => setField("is_featured", e.target.checked)} className="accent-[#C9A84C]" />
            <span className="text-sm text-gray-700">Featured on homepage</span>
          </label>
          <label className="flex items-center gap-2 mt-2 cursor-pointer">
            <input type="checkbox" checked={form.is_on_sale} onChange={(e) => setField("is_on_sale", e.target.checked)} className="accent-[#C9A84C]" />
        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded p-4">
          ...
          <div className="mt-3">
            <Field label="Focus Keywords">
              <input value={form.focus_keywords} onChange={(e) => setField("focus_keywords", e.target.value)}
                className={inputCls} placeholder="e.g. blue cut glasses, men frames" />
            </Field>
          </div>
          <div className="mt-3">
